...
define([
        ...
        C.CM("factual_basis_detail","html!"),
        C.CMF("router.js"),
    ...
    function(avalon, css, html, x, data_center,layer) {
        ...
        // 作品基本详细信息组件
        var detail = avalon.component('ms-factual_basis_detail', {
            template: html,
            defaults: {
                url_file:"",//获取文件
                url:"",//获取品德详情
                ...
                productGetDetalisById: function() { //get详细
                    this.data.id = data_center.get_key("factual_id");
                ...
                on_request_complete: function(cmd, status, data, is_suc, msg) {
                    if (is_suc) {
                        switch (cmd) {
                            case this.url:
                                complete_moralDevelopmentGetDetalisById(data);
                                break;
                        }
                    } else {
                        layer.msg(msg)
                ...
                //修正照片
                update_img:function () {
                    sessionStorage.setItem("mo_data_detail", JSON.stringify(this.data));
                    window.location="#mo_update_img";
                }
            }
        })
