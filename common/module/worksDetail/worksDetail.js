...
define([
    ...
    var detail = avalon.component('ms-base-detail', {
        template: html,
        defaults: {
            url_file:"",
            url:"",
            ...
            productGetPriductById: function() { //get详细
                this.data.id = data_center.get_key("works_id");
            ...
            on_request_complete: function(cmd, status, data, is_suc, msg) {
                if (is_suc) {
                    switch (cmd) {
                        case this.url:
                            complete_productGetPriductById(data);
                            break;
                    }
                } else {
                    layer.msg(msg)
            ...
            //修正照片
            update_img:function () {
                sessionStorage.setItem("works_data_detail", JSON.stringify(this.data));
                window.location="#works_update_img";
            }
        }
    });

    complete_productGetPriductById = function(data) {
