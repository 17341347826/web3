...
define([
        "jquery",
        C.CLF('avalon.js'),
        C.CM("comparative_analysis_area", "css!"),
        C.CM("comparative_analysis_area", "html!"),
    ...
    function ($, avalon, css, html, data_center, layer,highcharts,exporting,zh_CN,highcharts_more) {
        var vm = avalon.component('ms-comparative-analysis-area', {
            template: html,
            defaults: {
                //年级列表
                ...
                demand: function () {
                    this.tit = this.other.city + "-" + "" + this.other.grade_name + ""+ "-" + "" + this.other.semester_name + "";
                    // 请求对比数据
                        ...
                        switch (cmd) {
                            case this.grade_list_api:
                                ...
                                if (data.data.length != 0) {
                                    ...
                                    this.tit = this.other.city + "-" + "" + this.other.grade_name + "";
                                    //请求学年学期
                                    ajax_post(this.grade_opt_semester, {grade_id: data.data[0].id}, this);
                                    // 请求对比数据
                                    //this.abcd();
                                } else {
                                    toastr.warning("无年级数据！")
                                }
                                break;
                ...
                complete_contrast_abcd:function (data) {
                    ...
                    for(var i=0;i<list.length;i++){
                        var item = list[i].district;
                        chart_area.push(item);
                    ...
                    //图表高度
                    var len = this.chart_area.length;
