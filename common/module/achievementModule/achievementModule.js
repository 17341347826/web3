...
define([
    ...
    function(avalon, css, html, x, data_center,layer) {
        ...
        // 成就基本详细信息组件
        var detail = avalon.component('ms-base-achievement-detail', {
            template: html,
            defaults: {
               url_file:"",//获取文件
               url:"",//成就详细
                ...
                productGetDetalisById: function() { //get详细
                    this.data.id = data_center.get_key("achieve_id");
                ...
                on_request_complete: function(cmd, status, data, is_suc, msg) {
                    if (is_suc) {
                        switch (cmd) {
                            case this.url:
                                complete_getAchievementDetailById(data);
                                break;
                        }
                    } else {
                        layer.msg(msg)
        ...
        complete_getAchievementDetailById = function(data) {
            var dataList=data.data;
            var data_attachment=dataList.ach_enclosures;
