...
define([
        "jquery", 'layer',
        C.CLF('avalon.js'),
        C.Co("daily_performance", "publicity/art_evaluation_public/art_evaluation_public", "css!"),
        C.Co("daily_performance", "publicity/art_evaluation_public/art_evaluation_public", "html!"),
        C.CM("table"),
            ...
            var vm = avalon.define({
                ...
                complete_query_pub: function (data) {
                    ...
                    if (list != null && list.length > 0) {
                        //mkid（模块id）：1.日常表现2.综合实践活动3.成就奖励4.学业成绩5.体质健康测评6.标志性卡7.学期评价8.毕业评价9.艺术测评
                ...
                complete_get_info: function (data) {
                    ...
                    var stuListLength = stuList.length;
                    var tbody_score = obj_y;
                    for (var x = 0; x < stuListLength; x++) {
                        var student = stuList[x];
                        for (var i = 0; i < tbody_score.course_list.length; i++) {
                            var subject = tbody_score.course_list[i];
                            if (tbody_score.hasOwnProperty(student.code)) {
                                stuList[x][subject.key] = tbody_score[student.code][subject.key];
                                stuList[x]._id = tbody_score[student.code]._id
                            } else {

                                stuList[x][subject.key] = {
                                    "value": "",
                                    "score": "",
                                    "level": ""
                                };
                    ...
                    //获取到所有学科
                    // var course_value = obj_y.course_list;
                    // var table_head = [
                    //     {name: "序号", type: "index", from: "index"},
                    //     {name: "姓名", type: "text", from: "name"},
                    //     {name: "学籍号", type: "text", from: "code"}
                    // ];
                    // this.thead = table_head.concat(course_value);
                    // this.thead.push({
                    //     name: "操作",
                    //     type: "html",
                    //     from: "<a class='tab-btn tab-objection-btn' ms-click='@dissentClick(el)'  title='异议'></a>"
                    // });
                    //表头
