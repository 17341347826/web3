...
define([
        "jquery", 'layer',
        C.CLF('avalon.js'),
        C.Co("daily_performance", "publicity/graduation_report_dissent/graduation_report_dissent", "css!"),
        C.Co("daily_performance", "publicity/graduation_report_dissent/graduation_report_dissent", "html!"),
        C.CM("table"),
        ...
        var avalon_define = function () {
                var vm = avalon.define({
                        ...
                        rankChange: function () {
                            ...
                            //0.1透明度的白色背景
                            // this.index = layer.load(2, {shade:[0.3,'#121212']}); //0代表加载的风格，支持0-2
                        ...
                        complete_rank_count: function (data) {
                            ...
                            // this.index = layer.load(1, {shade:[0.3,'#121212']});;//0.1透明度的白色背景
                            // this.index = layer.load(2, {shade:[0.3,'#121212']}); //0代表加载的风格，支持0-2
                            //表头
                            ajax_post(get_title_api, {
                                grade_id: this.req_data.grade_id,
                        ...
                        complete_query_dissent: function (data) {
                            ...
                            if (data.data.length == 0) {
                                ...
                                console.dir(this.grade_list);
                                var grade_name = '';
                                for (var i = 0; i < this.grade_list.length; i++) {
                                    if (vm.audited_one.grade_id == this.grade_list[i].grade_id) {
                                ...
                                for (var j = 0; j < this.class_list.length; j++) {
                                    if (vm.audited_one.class_id == this.class_list[j].class_id) {
                                ...
                                layer.prompt({title: '请填写异议', formType: 2}, function (text, index) {
                                    if ($.trim(text) != "") {
                                        ajax_post(api_evaluation_dissent, {
                                            content: text,
                                            synthesize_id: id,
                                            ...
                                            "name": user.name,
                                            "fk_grade_id": vm.audited_one.grade_id,
                                            "grade_name": grade_name,
                                            "fk_class_id": vm.audited_one.class_id,
                                            "class_name": class_name,
                                            "student_num": vm.audited_one.stu_num,
                                            "student_name": vm.audited_one.stu_name
                                        }, self);
                                    }
                                    layer.close(index);
                                });
                            } else {
                                layer.alert('不能重复提交异议', {
                                    closeBtn: 0
                                    , anim: 4 //动画类型
                                });
                            }
