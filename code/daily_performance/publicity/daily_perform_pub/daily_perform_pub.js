...
define(["jquery",
        C.CLF('avalon.js'),
        'layer',
        C.Co('daily_performance', 'publicity/daily_perform_pub/daily_perform_pub','html!'),
        C.Co('daily_performance', 'publicity/daily_perform_pub/daily_perform_pub','css!'),
            ...
            var vm = avalon.define({
                ...
                scoll_load:function(){
                    $(window).scroll(function(){
                        var scrollTop = $(this).scrollTop();//滚动条距离网页顶部的高度
                        var scrollHeight = $(document.body).height();//网页文档的高度
                        var windowHeight = $(this).height();//页面可视化区域高度
                        if(Math.ceil(windowHeight+scrollTop)>scrollHeight  && vm.only_scoll_req){
                            if( vm.form_list_score.offset+vm.form_list_score.rows >= vm.all_count || vm.daily_list.length == 0){
                                // $(window).off("scroll");//结束滚动
                                return;
                            }
                            content_pl.start();
                            vm.data_had = false;
                            vm.form_list_score.offset =  vm.form_list_score.offset + vm.form_list_score.rows;
                            vm.old_scroll_top = scrollHeight;
                            vm.only_scoll_req = false;
                            //1s后在复活这个变量，避免了滚动一次重复请求接口
                            // setTimeout(function(){
                            //     vm.only_scoll_req = true;
                            // },1000);
                            //    日常表现列表
                            ajax_post(api_daily_pub,vm.form_list_score.$model,vm);
                        }else{
                        }
                    });
                },
                ...
                to_search:function(){
                    ...
                    this.data_had = false;
                    vm.form_list_score.offset = 0;
                    vm.form_list_score.xsmc = this.stu_name;
                    vm.form_list_score.xjh = this.stu_num;
                    vm.old_scroll_top = 0;
                    //    日常表现列表
                    ajax_post(api_daily_pub,this.form_list_score.$model,this);
                },
                init:function () {
                    ...
                    content_pl = new pl();
                    content_pl.init({
                        target: "#daily_perform_pub_content",
                ...
                ask_dis:function(id,idx){
                    ...
                    layer.prompt({title: '请填写异议', formType: 2}, function(text, index){
                        ...
                        vm.dissent_obj = {owner_name:vm.ident_name,content:text};
                ...
                deal_data:function(data){
                    ...
                    //     vm.only_scoll_req = true;
                    // },1000);
                    this.daily_list = this.daily_list.concat(list);
                    this.data_had = true;
                    content_pl.stop();
                },
                //提异议
                complete_comment_add:function(data){
                    ...
                    // //    日常表现列表
                    // ajax_post(api_daily_pub,this.form_list_score.$model,this);
                    this.daily_list[this.dissent_index].dissent_list.push(this.dissent_obj);
                },
            });
