define([
        ...
        'layer',
        "date_zh",
        C.Co("evaluation_material", "teacher_evaluation/evaluation_project_view/evaluation_project_view", "css!"),
        C.Co("evaluation_material", "teacher_evaluation/evaluation_project_view/evaluation_project_view", "html!"),
        C.CMF("router.js"),
            ...
            var vm = avalon.define({
                ...
                theadTh: [
                    ...
                    },
                    {
                        title: "操作",
                        type: "html",
                        from: "<a :if='el.pro_state!=2' class='tab-btn tab-edit-btn' ms-on-click='@oncbopt({current:$idx, type:1})' title='编辑'></a>" +
                        "<a class='tab-btn tab-details-btn' ms-on-click='@oncbopt({current:$idx, type:2})' title='查看'></a>" +
                        "<a :if='el.pro_state!=2' class='tab-btn tab-trash-btn' ms-on-click='@oncbopt({current:$idx, type:3})' title='删除'></a>"
                    }
                ],
                init: function () {
                ...
                cbopt: function (params) {
                    if (params.type == 1) {
                        ...
                        window.location = "#evaluation_project_create?project_id=" + params.data.id +
                            "&grade_id=" + pmx.grade_id + '&is_switch=' + pmx.is_switch + '&module_type=' + pmx.module_type + "&grade_name="+pmx.grade_name + "&plan_subjectid=" + pmx.plan_subjectid;
                        ...
                        //删除
                        this.delete_msg(params.data.id);
                ...
                create_project: function () {
                    window.location = "#evaluation_project_create?pro_rank=" + this.extend.pro_rank + "&plan_subjectid=" + this.extend.pro_type +
                ...
                go_href: function () {
                    window.location = "#city_management_create_scheme_list?plan_subjectid=" + this.extend.pro_type +
                        "&grade_id=" + pmx.grade_id + '&is_switch=' + pmx.is_switch + '&module_type=' + pmx.module_type + '&grade_name=' + pmx.grade_name;
                },
                //参数设置
