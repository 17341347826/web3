define(["jquery",
        ...
        C.Co("evaluation_material/stu_evaluation","project_content_fill/project_content_fill","css!"),
        C.Co("evaluation_material/stu_evaluation","project_content_fill/project_content_fill","html!"),
        C.CMF("router.js"),
        ...
        var avalon_define = function(pmx) {
            var vm = avalon.define({
                ...
                blur_input: function (el,idx) {
                    // var index_value = Number(el.pj_value);
                    // var value_list = Number(el.my_value);
                    // if (value_list < index_value) {
                    //     toastr.warning('不能超过最大分值');
                    //     el.pj_value = '';
                    // } else if (index_value < 0) {
                    //     toastr.warning('分值不能为负数');
                    //     el.pj_value = '';
                    // }


                    if($.trim(el.pj_value)==''){
                        this.student_arr[idx].pj_value = '';
                    ...
                    if(parseFloat(el.pj_value)<0){
                        this.student_arr[idx].pj_value = 0;
                        return;
                    }
                    if(parseFloat(el.pj_value)>parseFloat(el.my_value)){
                        this.student_arr[idx].pj_value = el.my_value;
                        return;
                    }
                },
                get_info:function () {
                ...
                complete_get_student:function (data) {
                    ...
                    for(var i=0;i<dataListLength;i++){
                        ...
                        dataList[i]['pj_cover_name_num'] = dataList[i]['code'];
                ...
                get_concat_stu:function(ary1,ary2,name){
                    for(var i=0,len=ary1.length;i<len;i++){
                        if(vm.get_ary_element(ary2,name,ary1[i].pj_cover_name_guid)){
                ...
                complete_api_index_max:function (data) {
                    ...
                    else if(this.remark == 2){//之前吗没有评论过
                ...
                value_click:function () {//pj_value
                    ...
                    if(name_arr_length>0){
                        ...
                        layer.confirm(name_str+'这'+'【'+name_arr_length+'】'+'位同学分值不符合规范,请检查并更正', {
                            btn: ['知道了'] //按钮
                        }, function(){
                            layer.closeAll();
                        });
