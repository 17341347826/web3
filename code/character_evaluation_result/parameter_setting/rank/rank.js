define(
    [
        C.CLF('avalon.js'),
        "layer",
        C.Co('character_evaluation_result/parameter_setting', 'rank/rank', 'html!'),
        C.Co('character_evaluation_result/parameter_setting', 'rank/rank', 'css!'),
        C.Co('character_evaluation_result/parameter_setting', 'content_set/content_set', 'css!'),
        C.CMF("router.js"),
        C.CMF("data_center.js"),
        C.CM("three_menu_module")
    ],
    function (avalon, layer, html, css,css2, x, data_center, three_menu_module) {
        var avalon_define = function () {
            ...
            var vm = avalon.define({
                ...
                grade_index:0,
                default_value:'',
                inputLimits:function (e, idx, score_lv, el2) {
                    if (idx === '') {
                ...
                init: function () {
                    ...
                    this.rule_extend.fk_grade_id = this.grade_list[this.grade_index].value.toString();
                    if (data_center.get_key('now_grade_id')) {
                        this.rule_extend.fk_grade_id = data_center.get_key('now_grade_id');
                    }
                    this.rule_extend.unit_lv = cloud.user_level();
                    this.rule_extend.fk_unit_id = cloud.user_depart_id();
                ...
                change_state: function () {
                    if (this.division_method == '' || this.standard == '') {
                        toastr.warning('划分方式和标准必选');
                        return;
                    }
                    if (this.level_count == 1) {
                        toastr.warning('至少设置两个等级');
                        return;
                    }
                    ...
                    if (this.lv_type == 1 || this.lv_type == 2 || this.lv_type==5) {
                        if (this.level_list.full_score == '') {
                            ...
                            toastr.warning('请先设置总分！');
                            return;
                        }
                        if (this.level_list.full_score == 0) {

                            toastr.warning('总分不能为零');
                            return;
                        }
                        ...
                        for (var i = 0; i < lv_partition_length; i++) {
                            ...
                            if (first_tall_score != this.level_list.full_score) {
                                toastr.warning(name+':A等级最高分应该和设置的最高分相等！');
                                return;
                            }
                            if (last_low_score != 0) {
                                toastr.warning(name+ ':最后一个等级最低分必须为0！');
                                return;
                            }
                            for (var j = 0; j < score_lv_length - 1; j++) {
                                ...
                                if (low_score != tall_score) {
                                    toastr.warning(name + ':上一个等级最低分必须和下一个等级最高分相等！');
                                    return;
                                }
                            }
                        }
                    ...
                    if (this.lv_type == 3 || this.lv_type == 4 || this.lv_type==6) {
                        for (var i = 0; i < lv_partition_length; i++) {
                            ...
                            if (now_school_all_ratio != 100) {
                                toastr.warning(now_name + ':总比例必须为100！');
                                return;
                            }
                        }
                    }
                ...
                recombination: function (lv_type) {
                    ...
                    if(this.lv_type==5 || this.lv_type==6){
                        if (lv_partition_length == 0 || (lv_partition_length == 1 && this.level_list.lv_partition[0].fk_class_id == -1)) {
                            ...
                            if(this.level_list.lv_partition.length==0){
                                layer.msg('该年级无班级信息');
                            }
                        }
                    }
