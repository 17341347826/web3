define([
        C.CLF('avalon.js'),
        'layer',
        C.Co('daily_evaluation_statistics', 'daily_special/daily_special', 'html!'),
            ...
            var vm = avalon.define({
                ...
                presentation_change:function (num) {
                    ...
                    switch (dis){
                        case 1:
                            break;
                        case 2:
                            this.to_page('daily_comprehensive_practice_wd?sta_type=2&module_type=8&page=not_school');
                            break;
                        case 3:
                            this.to_page('daily_comprehensive_practice_wd?sta_type=3&module_type=8&page=not_school');
                            break;
                        default:
                            break;
                    }
                ...
                deal_data: function (data, type, div_id) {
                    ...
                    for (var i = data_length - 1; i > -1; i--) {
                        ...
                        if (type == 'city') {
                            y_name = data[i].city;
                            sczb = Number(data[i].city_cnt.rc_pjf) + Number(data[i].city_cnt.gx_pjf);
                        } else if (type == 'district') {
                            sczb = Number(data[i].district_cnt.rc_pjf) + Number(data[i].district_cnt.gx_pjf);
                            y_name = data[i].district;
                        } else {
                            y_name = data[i].schoolname;
                            sczb = Number(data[i].detail.school_cnt.rc_pjf) + Number(data[i].detail.school_cnt.gx_pjf);
                        }
                        if(!sczb)
                            sczb = 0;
                        sczb = sczb.toFixed(2)
