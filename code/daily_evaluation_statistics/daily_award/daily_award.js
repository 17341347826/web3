define([
        C.CLF('avalon.js'),
        'layer',
        C.Co('daily_evaluation_statistics', 'daily_award/daily_award', 'html!'),
            ...
            var vm = avalon.define({
                ...
                presentation_change:function (num) {
                    ...
                    switch (dis){
                        case 1:
                            break;
                        case 2:
                            this.to_page('daily_comprehensive_practice_wd?sta_type=2&module_type=3&page=not_school');
                            break;
                        case 3:
                            this.to_page('daily_comprehensive_practice_wd?sta_type=3&module_type=3&page=not_school');
                            break;
                        default:
                            break;
                    }
                ...
                deal_data: function (data, type, div_id) {
                    ...
                    for (var i = data_length - 1; i > -1; i--) {
                        ...
                        if (type == 'city') {
                            y_name = data[i].city;
                            sczb = Number(data[i].city_cnt.pj_df);
                        } else if (type == 'district') {
                            y_name = data[i].district;
                            sczb = Number(data[i].district_cnt.pj_df);
                        } else {
                            y_name = data[i].schoolname;
                            sczb = Number(data[i].detail.school_cnt.pj_df);
                        }
                        if (!sczb)
                            sczb = 0;
                ...
                deal_msg: function (data) {
                    ...
                    if (data.data.city_cnt) {
                        ...
                        if (data.data.city_cnt.length == 0) {
                            this.city_process_list = [];
                            this.deal_data(this.filter_city(this.city_process_list), 'city', 'tubiao_1');
                        } else {
                            ...
                            var obj = this.get_default(city_cnt[0]);
                            student_city = merge_table(student_city, ["grade_id"], city_cnt, ["fk_grade_id"], "city_cnt", obj);
                            sort_by(student_city, ["+city", "+district", "+school_id", "+grade_name"]);

                            this.city_process_list = student_city;
                    ...
                    if (data.data.district_cnt) {
                        ...
                        if (data.data.district_cnt.length == 0) {
                            ...
                            var obj = this.get_default(district_cnt[0]);
                            student_area = merge_table(student_area, ["district", "grade_id"], district_cnt, ["district_name", "fk_grade_id"], "district_cnt", obj);
                            sort_by(student_area, ["+city", "+district", "+school_id", "+grade_name"]);
                            this.area_process_list = student_area;
                            if (this.user_level < 4)
                                this.deal_data(this.filter_area(this.area_process_list), 'district', 'tubiao_2');
                        }
                    ...
                    if (data.data.school_cnt) {
                        ...
                        if (data.data.school_cnt.length == 0) {
                            ...
                            student = merge_table(student, ["school_id", "grade_id"], school_cnt, ["fk_school_id", "fk_grade_id"], "school_cnt", obj);

                            this.sublist = JSON.parse(JSON.stringify(student[0].school_cnt.sub_list));
                            this.sublist_count = this.sublist.length;
                            var arr = [];
