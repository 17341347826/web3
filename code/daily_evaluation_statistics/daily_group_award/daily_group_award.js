define([
        C.CLF('avalon.js'),
        'layer',
        C.Co('daily_evaluation_statistics', 'daily_group_award/daily_group_award', 'html!'),
    ...
    function (avalon, layer, html, css, data_center, select_assembly, echarts, three_menu_module) {
        var zsqk_api = api.growth + "award_winning_situation_sta_analysis_zsqk";
        var sex_api = api.growth + "award_winning_situation_sta_analysis_sex";
        var user = {};
        ...
        var avalon_define = function () {
            ...
            var vm = avalon.define({
                ...
                presentation_change:function (num) {
                    ...
                    switch (dis){
                        case 1:
                            break;
                        case 2:
                            this.to_page('daily_group_wd?sta_type=2&module_type=3');
                            break;
                        case 3:
                            this.to_page('daily_group_ys?sta_type=3&module_type=3');
                            break;
                        default:
                            break;
                    }
                ...
                deal_cnt:function (data) {
                    for(var i=0;i<data.length;i++){
                        data[i].cjjb_arr = data[i].cjjb.split(',');
                        data[i].rjjl_arr = data[i].rjjl.split(',');
                        if(data[i].cjjb_arr[data[i].cjjb_arr.length-1]==''){
                            data[i].cjjb_arr.pop();
                        }
                        if(data[i].rjjl_arr[data[i].rjjl_arr.length-1]==''){
                            data[i].rjjl_arr.pop();
                        }
                    }
                    var arr = [];
                    for(var i=0;i<data.length;i++){
                        var cjjb_arr = data[i].cjjb_arr;
                        for(var j=0;j<cjjb_arr.length;j++){
                           if(arr.indexOf(cjjb_arr[j])==-1){
                               arr.push(cjjb_arr[j])
                           }
                        }
                    }
                    for(var i=0;i<data.length;i++){
                        if(data[i].cjjb_arr.length<arr.length){
                            var new_arr = data[i].cjjb_arr;
                            for(var k=0;k<arr.length;k++){
                                var key = arr[k];
                                if(!new_arr[k] || new_arr[k]!=key){
                                    data[i].cjjb_arr.splice(k,0,key)
                                    data[i].rjjl_arr.splice(k,0,0)
                                }
                            }
                        }
                    }
                ...
                get_title_arr:function (data) {
                    var title_arr = [];
                    for(var i=0;i<data.length;i++){
                        if(data[i].detail.cjjb_arr!=0&&data[i].detail.cjjb_arr.length>0){
                            title_arr = data[i].detail.cjjb_arr;
                            break;
                        }
                    }
                    return title_arr;
                ...
                deal_msg: function (data) {
                    ...
                    if (data.data.city_cnt) {
                        this.city_process_list = this.deal_cnt(data.data.city_cnt);
                        var is_city_leader = cloud.is_city_leader();
                        if (is_city_leader)
                    ...
                    if (data.data.district_cnt ) {
                        ...
                        if (this.now_type == 1) {
                            sort_by(district_cnt, ['+district_name', '+stu_sex'])
                        }
                        if (this.now_type == 2) {
                    ...
                    if (data.data.school_cnt) {
                        ...
                        var new_cnt = DailyAnaly.complate_data(school_cnt,['school_name','school_id','district_name','grade_id'],key2,arr,0);
                        sort_by(new_cnt, ['+school_id', '+status'])

                        if (new_cnt.length > 0 && this.user_level < 4){
                            vm.school_p.district_name = new_cnt[0].district_name;
                            this.defalut_area_school = new_cnt[0].district_name;
                        }
                    ...
                    if (data.data.school_class_cnt ) {
                        ...
                        var new_cnt = DailyAnaly.complate_data(school_class_cnt,['grade_name','grade_id','class_id','class_name'],key2,arr,0);
                        sort_by(new_cnt, ['+grade_id', '+class_id', '+status'])
                        this.title_arr_class = this.get_title_arr(new_cnt)
