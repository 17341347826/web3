define([
        C.CLF('avalon.js'),
        'layer',
        C.Co('daily_evaluation_statistics', 'daily_special_school/daily_special_school', 'html!'),
    ...
    function (avalon, layer, html, css, data_center, select_assembly, echarts, three_menu_module,formatUtil) {
        ...
        var grade_ids = [];
        var msg_api = api.api + "GrowthRecordBag/daily_perfor_and_personality_eval_sta_analysis";
        var avalon_define = function () {
            var ori_grade_list = [];
            var vm = avalon.define({
                $id: "daily_special_school",
                ...
                presentation_change:function (num) {
                    ...
                    switch (dis){
                        case 1:
                            break;
                        case 2:
                            this.to_page("#daily_comprehensive_practice_wd?sta_type=2&module_type=8&page=school");
                            break;
                        case 3:
                            this.to_page("#daily_comprehensive_practice_wd?sta_type=3&module_type=8&page=school");
                            break;
                        default:
                            break;
                    }
                ...
                draw_bar: function (div_id, table_data,type) {
                    ...
                    for (var i = table_list_length-1; i > -1; i--) {
                        ...
                        var new_wcl = 0;
                        if(type=='grade'){
                            new_wcl = Number(table_data[i].school_cnt.rc_pjf)+Number(table_data[i].school_cnt.gx_pjf)
                        }
                        if(type=='class'){
                            new_wcl = Number(table_data[i].school_class_cnt.rc_pjf)+Number(table_data[i].school_class_cnt.gx_pjf);
                        }
                        if(!new_wcl)
                            new_wcl = 0;
                ...
                deal_msg:function (data) {
                    ...
                    if (data.data.school_cnt) {
                        ...
                        student_school = merge_table(student_school, ["grade_id"], data.data.school_cnt, ["fk_grade_id"], "school_cnt",0);
                            ...
                            cloud.sem_class_list({fk_nj_id: each.value, fk_xq_id: Number(vm.semester.split('|')[0])},
                                function (url, arg, data) {
                                    ...
                                    if(class_load_count==vm.grade_list.length-1){
                                        class_count = merge_table(class_count, ["fk_nj_id","fk_bj_id"], school_class_cnt, ["fk_grade_id","fk_class_id"], "school_class_cnt",0);
                                        sort_by(class_count, ["+njmc","+bjmc"]);
                                        console.dir(class_count)
