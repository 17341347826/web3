...
define(["jquery",
        C.CLF('avalon.js'),
        'layer',
        C.Co('self_evaluation_management', 'self_compre_practice/compre_practice_see/compre_practice_see', 'html!'),
        C.Co('self_evaluation_management', 'self_compre_practice/compre_practice_see/compre_practice_see', 'css!'),
    ...
    function ($, avalon, layer, html, css, data_center, select_assembly, three_menu_module) {
        ...
        var avalon_define = function () {
            var vm = avalon.define({
                ...
                //模块集合
                module_list: [
                    {id: 1, module_name: '思想品德', cout: 0, mk: 1},
                    {id: 2, module_name: '学业水平', cout: 0, mk: 4},
                    {id: 3, module_name: '身心健康', cout: 0, mk: 5},
                    {id: 4, module_name: '艺术素养', cout: 0, mk: 2},
                    {id: 5, module_name: '社会实践', cout: 0, mk: 3},
                ],
                //前一次请求的滚动条高度
                old_scroll_top: '',
                //当前数据总数
                ...
                semester_remark: -1,
                open_index: [],
                open_close_detail: function (index) {
                    const new_index = this.open_index.indexOf(index);
                    if (new_index == -1) {
                        this.open_index.push(index);
                        console.log(this.open_index.indexOf(index) != 1, this.open_index )
                        return;
                    }
                    this.open_index.splice(new_index, 1);
                    console.log(this.open_index.indexOf(index) != 1, 312)
                },
                url_for: function (id) {
                ...
                listen_scroll: function () {
                    ...
                    $(window).scroll(function () {
                        ...
                        if (scrollTop + windowHeight == scrollHeight && self.current_extend.offset+self.current_extend.rows<self.current_count) {
                            self.current_extend.offset = self.current_extend.offset+self.current_extend.rows;
                            self.get_list_data();
                        }
                    })
                },
                //模块切换
                click_module_index: function (index) {
                    //初始化列表
                    this.current_list = [];
                ...
                deal_data: function (data) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        data[i].show_open_close = false;
                        var fjdz = JSON.parse(data[i].fjdz);
                        if (fjdz.length > 0) {
                            data[i].show_open_close = true
                        }
                        data[i].img_arr = [];
                        data[i].video_arr = [];
                        data[i].file_arr = [];
                ...
                get_module_count: function () {
                    for (var i = 0, len = this.sem_count.length; i < len; i++) {
                        for (var j = 0, len2 = this.module_list.length; j < len2; j++) {
                            if (this.sem_count[i].mk == this.module_list[j].mk) {
                                this.module_list[j].cout =this.sem_count[i].num;
                                break;
                            }
                        }
                    }
