...
define(["jquery",C.CLF('avalon.js'),'layer',"date_zh",
        C.Co("evaluate_students","peer_review/student_mutual_evaluation/student_mutual_evaluation","css!"),
        C.Co("evaluate_students","peer_review/student_mutual_evaluation/student_mutual_evaluation","html!"),
        C.CMF("router.js"),C.CMF("data_center.js"),C.CM("table"),
        C.CM('three_menu_module'),C.CM("select_assembly")],
    function($,avalon, layer,date_zh,css, html, x, data_center,page_title,tab,three_menu_module,select_assembly) {
        //获取年级
            ...
            var vm = avalon.define({
                ...
                theadTh: [{
                    ...
                    },
                    {
                        ...
                        type: "html",
                        from:
                        "<a :if='el.pro_state!=2 && params.user_type == 0' class='tab-btn tab-edit-btn' ms-on-click='@oncbopt({current:$idx, type:1})' title='编辑'></a>" +
                        "<a class='tab-btn tab-details-btn' ms-on-click='@oncbopt({current:$idx, type:2})' title='查看'></a>" +
                        "<a :if='params.user_type == 0' class='tab-btn tab-trash-btn' ms-on-click='@oncbopt({current:$idx, type:3})' title='删除'></a>"+
                        //让审核通过的项目也可以删除
                        // "<a :if='el.pro_state!=2 && params.user_type == 0' class='tab-btn tab-trash-btn' ms-on-click='@oncbopt({current:$idx, type:3})' title='删除'></a>"+
                        "<a class='tab-btn tab-statistics-btn' :if='params.user_type == 0' ms-on-click='@oncbopt({current:$idx, type:5})' title='统计'></a>"
                    }
                ],
                cb: function() {
                    var self = this;
                ...
                //创建
                self_add:function(){
                    window.location='#student_add_mutual_evaluation?grade_id='+pmx.grade_id+
                ...
                add_programme:function () {
                    var id="";
                    data_center.set_key("get_mutual_id",id);
                    window.location="#student_add_mutual_evaluation?grade_id="+pmx.grade_id+
                    '&is_switch='+pmx.is_switch+'&module_type='+pmx.module_type;
                },
                cbopt: function (params) {
                    ...
                    data_center.set_key("get_mutual_id", id);
                    if(params.type == 1) {//修改
                        window.location="#student_add_mutual_evaluation?params_type="+1+"&get_mutual_id="+id+'&grade_id='+pmx.grade_id+
                            '&is_switch='+pmx.is_switch+'&module_type='+pmx.module_type;
                    }else if(params.type==2){//详情
