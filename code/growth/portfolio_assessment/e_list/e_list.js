...
define([
        C.CLF('avalon.js'),
        'layer',
        C.Co('growth', 'portfolio_assessment/e_list/e_list', 'html!'),
        C.Co('growth', 'portfolio_assessment/e_list/e_list', 'css!'),
            ...
            var vm = avalon.define({
                ...
                cbopt: function (p) {
                    ...
                    cloud.has_pjda({ fk_nj_id: vm.extend.fk_grade_id, fk_xq_id: vm.extend.semester,
                        guid: p.data.guid, status: 5
                    }, function (url, args, data) {
                        if (data.length == 0) {
                            toastr.warning("该学生还未生成学期评价数据")
                            return;
                        }
                        ...
                        if (p.type == 1) {
                            ...
                            var href = "guid=" + p.data.guid + "&s=" + start_date + "&e=" + end_date +
                                "&sid=" + sid + "&smsid=" + vm.extend.semester;
                            data_center.set_key('export_data', JSON.stringify(vm.export_extend))
                            window.location = '#evaluation_detail?' + href;
                            return;
                        } else if (p.type == 2) {
                            vm.grade_distinguish();
                        }
                    });

                },
                grade_distinguish: function () {
                    ajax_post(grade_class_api, {id: this.extend.fk_grade_id + ''}, this)
                },
                //单个导出
                get_extend: function (p) {
                    this.export_extend.token = sessionStorage.getItem('token')
                    var semester_arr = this.sem_info.split('|');
                    this.export_extend.semester_id = semester_arr[0];

                    this.export_extend.project_id = this.extend.pro_id;

                    for (var i = 0; i < this.semester_list.length; i++) {
                        if (this.export_extend.semester_id == this.semester_list[i].id) {
                            this.export_extend.phase = this.semester_list[i].semester_index;
                            this.export_extend.start_time = time_2_str(this.semester_list[i].start_date);
                            this.export_extend.end_time = time_2_str(this.semester_list[i].end_date);
                            this.export_extend.year = this.export_extend.start_time.substr(0, 4);
                            break;
                        }
                    }
                    if (p) {
                        this.export_extend.guid = p.data.guid;
                        this.export_extend.school_id = p.data.fk_school_id;
                        this.export_extend.grade_id = p.data.fk_grade_id;
                        this.export_extend.class_id = p.data.fk_class_id;
                    }
                },
                ca_workid: D("user.user.fk_school_id"),
                //批量导出
                export_all: function () {
                    this.get_extend();
                    this.export_extend.school_id = this.extend.fk_school_id;
                    this.export_extend.grade_id = this.extend.fk_grade_id;
                    // this.export_extend.class_id = this.extend.fk_class_id;
                    this.export_extend.class_id = this.class_info;
                    delete  this.export_extend['guid'];
                    if (this.export_extend.project_id == '') {
                        layer.alert('请先选择项目');
                        return;
                    }
                    this.grade_distinguish();
                ...
                cds: function () {
                    ...
                    data_center.uin(function (data) {
                        ...
                        if (highest_level == 4) {//校
                            ...
                            self.class_list = vm.grade_list[0].class_list;
                            // self.only_hash = true;
                            self.area_info = tUserData.district;
                            self.extend.district = tUserData.district;
                            self.extend.fk_school_id = tUserData.fk_school_id;
