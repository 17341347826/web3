...
define([
        "jquery",
        C.CLF('avalon.js'),
        C.Co("health","health_project_mana/health_project_mana","css!"),
        C.Co("health","health_project_mana/health_project_mana","html!"),
        ...
        var avalon_define = function (pmx) {
            var health_table = avalon.define({
                ...
                theadTh:[
                    ...
                    "<a :if='el.check_status == 1 && el.fk_school_id == params.work_id && params.ident_type == 0' class='tab-btn tab-edit-btn' ms-on-click='@oncbopt({current:$idx, type:1})' title='编辑'></a>"+
                    // "<a :if='el.check_status == 2' class='tab-btn tab-edit-btn-disabled' ms-on-click='@oncbopt({current:$idx, type:1})' title='编辑'></a>"
                    "<a :if='el.check_status == 2 || params.ident_type == 1' class='tab-btn tab-edit-btn-disabled' title='不可编辑'></a>"
                ...
                create_scheme:function(){
                    window.location='#health_project_new?grade_id='+pmx.grade_id+'&is_switch='+pmx.is_switch+
                    '&module_type='+pmx.module_type;
                },
                //测评项目
