define(
    [
        C.CLF('avalon.js'),
        "layer",
        C.Co('eval_param_set', 'term_report_parameters/update_level/update_level','html!'),
        C.Co('eval_param_set', 'term_report_parameters/update_level/update_level','css!'),
            ...
            var vm = avalon.define({
                ...
                split_method_change:function () {
                    if(this.split_method==this.add_data.c_rankway && this.strand==this.add_data.c_standard){
                        ajax_post(api_get_find_by_count_rank,{c_gradeid:this.grade_id},this);
                    }else{
                    ...
                    if(this.split_method==1){//按分数段设置等级
                        ...
                        if(this.strand==1){//按年级
                            ...
                            if(b==this.level_list.length-1){
                                ajax_post(api_update,form,this);
                            }
                        }else{//按各班
                            var every_arr=[];
                            var x=0;
